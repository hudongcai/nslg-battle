...
app.post('/api/local-helper/tasks/:id/progress', requireHelperClient, async (req, res) => {
  try {
    ...
    const nextStats = parseTaskStats(stats);
    ...
    if (currentStatus === 'running' && nextStatus === 'error' && nextStats.pending > 0) {
      nextStatus = 'running';
    }
